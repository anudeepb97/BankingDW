...
getSQLServerData <- function() {
    ...
    db_df <- sqlQuery(con, 'select e.FIRST_NAME as fname,avg(a.AVAIL_BALANCE) as total
from dimAccount as a,dimEmployee as e
where a.OPEN_EMP_ID = e.EMP_ID and a.AVAIL_BALANCE>10000
...
for (i in 1:(n - windowQuery)) {
    ...
    plot(data$fname, data$total, type = "h", main = 'AVG BALANCE GREATER THAN')
    Sys.sleep(0.5)
}